...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < scratchcard_count; i++) {
		...
		while (scratchcard->copy_count) {
			...
			while (copies_written < copy_count) {
				for (int j = 0; j < scratchcard_count; j++) {
					Scratchcard *copy = &scratchcards[j];

					if (copy->id == scratchcard->id + copies_written + 1) {
						copy->copy_count++;
						break;
					}
				}
