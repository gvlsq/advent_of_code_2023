...
static u64 initial_seed_count;
static u64 initial_seeds[1 << 5];
...
static void preprocess_initial_seeds(Stream *stream) {
    ...
    u64 tmp;
    int chars_read;
    while (sscanf(stream->at, "%I64u%n", &tmp, &chars_read) == 1) {
        assert(initial_seed_count < array_count(initial_seeds));
        initial_seeds[initial_seed_count++] = tmp;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < initial_seed_count; i++) {
        u64 source_value = initial_seeds[i];
