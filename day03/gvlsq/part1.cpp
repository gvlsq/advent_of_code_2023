...
static Vector2i symbol_positions[1 << 11];
...
int main(int argc, char **argv) {
    ...
    while (*at) {
        ...
        for (int i = 0; i < line_length; i++) {
            if (line[i] != '.' && !is_numeric(line[i])) {
                assert(symbol_count < array_count(symbol_positions));
                symbol_positions[symbol_count++] = vector2i(i, line_index);
            }
        }
    ...
    // Sum part numbers who touch symbols
    int sum = 0;
    ...
    for (int i = 0; i < part_number_count; i++) {
        ...
        for (int j = 0; (j < value_length) && !valid; j++) {
            for (int k = 0; k < symbol_count; k++) {
                Vector2i symbol_position = symbol_positions[k];

                if (is_adjacent(vector2i(x + j, y), symbol_position)) {
                    valid = true;
                    break;
                }
            }
