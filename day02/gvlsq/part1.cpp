...
static bool parse_reveal(Tokenizer *tokenizer) {
    bool valid = true;
        ...
        switch (peek.type) {
            case TOKEN_TYPE_INTEGER: {
                ...
                if (strings_match(color.text, "red")) {
                    max = MAX_RED_CUBE_COUNT;
                } else if (strings_match(color.text, "green")) {
                    max = MAX_GREEN_CUBE_COUNT;
                } else if (strings_match(color.text, "blue")) {
                    max = MAX_BLUE_CUBE_COUNT;
                }

                if (count.integer_value > max) {
                    valid = false;
                    parsing = false;
                }
            } break;
...
static int parse_game(Tokenizer *tokenizer) {
    ...
    while (parsing) {
        ...
        switch (token.type) {
            case TOKEN_TYPE_INTEGER: {
                bool valid = parse_reveal(tokenizer);
                if (!valid) { // Even a single error means we give up on the whole game!
                    result = 0;
                    skip_line(tokenizer);
