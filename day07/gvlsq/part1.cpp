...
static inline Card_Group *find_group(Card_Group *groups, int group_count, char c) {
    Card_Group *result = 0;
...
static void group_by_cards(Hand *hand, Card_Group *groups, int *group_count) {
    // Raw grouping
    for (int i = 0; i < string_length(hand->cards); i++) {
        char c = hand->cards[i];

        Card_Group *group = find_group(groups, *group_count, c);
        if (!group) {
...
static Hand_Type compute_hand_type(Hand *hand) {
    ...
    group_by_cards(hand, card_groups, &card_group_count);
